  ...
  >
    <div class="bg-white rounded-lg p-6 w-full max-w-3xl shadow-lg">
      <div class="flex justify-between items-center border-b pb-3">
        <h2 class="text-xl font-bold">Registrar Técnico</h2>
        <button
          @click="closeModal"
      ...
      try {
        ...
        if (response.status === 201) {
          await Swal.fire({
            ...
            text: "El técnico se ha registrado exitosamente",
          });
          emit("close");
          emit("refresh"); // Para actualizar la lista
        } else {
          throw new Error("Error al registrar técnico");
        }
      } catch (error) {
        ...
        await Swal.fire({
          icon: "error",
          title: "Error",
          text: "Se produjo un error al registrar el técnico.",
        });
      }
    };
